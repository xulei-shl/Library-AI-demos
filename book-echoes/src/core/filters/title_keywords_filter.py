...
from src.utils.rule_file_parser import RuleFileParser
...
class TitleKeywordsFilter(BaseFilter):
    ...
    def apply(self, data: pd.DataFrame) -> Tuple[pd.DataFrame, Dict[str, Any]]:
        ...
        # 使用统一解析器加载关键词
        keywords = RuleFileParser.load_title_keywords(self.config['file'])
        if not keywords:
            return result_data, {
                'status': 'skipped',
                'reason': '没有加载到任何筛选关键词'
        ...
        for keyword in keywords:
            excluded_mask |= self._match_pattern(
                result_data[target_column].astype(str), keyword, match_type
            )

        excluded_count = excluded_mask.sum()
        result_data = result_data[~excluded_mask]

        logger.info(f"规则B - 题名关键词筛选: 排除 {excluded_count} 条记录 (关键词数: {len(keywords)})")
        ...
        return result_data, {
            ...
            'patterns_count': len(keywords),
            'excluded_count': excluded_count,
            'excluded_ratio': excluded_count / len(data) if len(data) > 0 else 0,
            'patterns_used': keywords[:10]  # 记录前10个使用的关键词
        }
