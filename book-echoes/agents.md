...
- **行动**: 
	...
	2. 必须首先运行“智能需求理解与研发导航引擎”模块，完成需求建模后再进行文档撰写。
	3. 如果涉及前端界面，必须同时加载 `@.rules/05_UIUX_DESIGNER.md` 进行布局规划。
	4. **拒绝直接编码**。
	5. 启动需求澄清与文档撰写流程。
...
- **行动**:
  ...
  2. 涉及 UI 实现时，严格遵守 `@.rules/05_UIUX_DESIGNER.md` 的视觉规范。
  3. 严格遵守代码工艺标准进行编码。
  4. **注意**: 如果用户请求复杂且未提供设计上下文，请强制降级路由至【架构师模式】。
...
### 5. UI 优化 / 视觉改进 -> 激活【UI/UX 专家模式】
- **触发条件**: 用户提到“美化界面”、“改进交互”、“界面太乱”、“优化视觉层级”等。
- **行动**:
	1. 引用 `@.rules/05_UIUX_DESIGNER.md`。
	2. 按照“视觉层级、对比、对齐、留白”等维度对现有界面进行审美审计（UI Audit）。
	3. 提出改进建议，并在用户确认后进入编码。
