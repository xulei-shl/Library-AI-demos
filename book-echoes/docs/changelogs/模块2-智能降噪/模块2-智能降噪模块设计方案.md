...
rule_a:
  enabled: true
  target_column: "近三个月总次数"  # 明确指定统计列名
  methods:
    # 方法1：分位数方法（推荐）
    percentile:
      enabled: true
...
def calculate_dynamic_threshold(data: pd.DataFrame, percentile: float = 15) -> int:
    ...
    # 获取非零借阅次数的索书号
    non_zero_data = data[data['近三个月总次数'] > 0]
    
    if len(non_zero_data) == 0:
        return 20  # 默认后备值
    
    # 使用分位数计算阈值
    threshold = np.percentile(non_zero_data['近三个月总次数'], 100 - percentile)
    
    # 如果分位数结果过小，使用统计特征
    if threshold < 5:
        threshold = np.percentile(non_zero_data['近三个月总次数'], 75)
...
- `智能降噪筛选报告_{timestamp}.txt` - 筛选过程统计报告
...
可用Excel列: [题名, 索书号, 附加信息, 备注, 类型/册数, 近三个月总次数]
