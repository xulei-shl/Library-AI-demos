...
config/
├── gramophone.yaml               # 模块8专属配置（新建）
└── setting.yaml                  # 全局配置（新增gramophone节点）
...
flowchart TD
    ...
    C -->|配置| C1[config/gramophone.yaml<br/>literary_filter_pattern]
    D -->|配置| D1[config/gramophone.yaml<br/>min_rating, required_fields]
    E -->|配置| E1[config/gramophone.yaml<br/>monthly_quota, sort_strategy]
```
...
2. **质量过滤**：
   ```python
   # 评分阈值（按学科配置）
   min_rating = config.get('gramophone.min_rating.I', 7.3)
...
4. **配额控制**：
   ```python
   monthly_quota = config.get('gramophone.monthly_quota', 8)
...
gramophone:
  ...
  # 配置文件路径
  config_file: "config/gramophone.yaml"
...
#### GramophonePipeline（主流程编排器）
```python
class GramophonePipeline:
    """模块8主流程编排器"""
...
sequenceDiagram
    ...
    participant Pipeline as GramophonePipeline
...
#### tests/test_gramophone_pipeline.py
```python
def test_pipeline_end_to_end():
    """测试完整流程"""
    pipeline = GramophonePipeline()
