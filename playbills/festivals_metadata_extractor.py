    ...
    for filename in os.listdir(output_folder):
        if filename.endswith('_ocr_1_final_result.md'):
            ...
            if processed_count % 2 == 0:
                ...
                print(f"已保存处理进度（共处理 {processed_count} 个文件）")
                logger.info(f"已保存处理进度（共处理 {processed_count} 个文件）")
    
    # 所有文件处理完成后最终保存一次
    if not df.empty:
        df.to_excel(excel_file_path, index=False)
        print(f"所有处理完成，结果已保存到 {excel_file_path}")
        logger.info(f"所有处理完成，结果已保存到 {excel_file_path}")