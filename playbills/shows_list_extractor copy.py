...
def select_model(token_count, env_vars):
    if token_count <= 32768:
       return env_vars["OneAPI_API_URL"], env_vars["OneAPI_API_KEY"], "doubao-1-5-pro-32k-250115"
        # return env_vars["DeepSeek_API_URL"], env_vars["DeepSeek_API_KEY"], "deepseek-chat"    
    elif token_count <= 131072:
        return env_vars["OneAPI_API_URL"], env_vars["OneAPI_API_KEY"], "doubao-1-5-pro-256k-250115"
...
def get_shows(md_file_path, logger, performing_event):
    ...
    event_info = f"Event: {performing_event['PerformanceEvent']['name']}"
    ...
    if any(item.get('eventCast') for item in result_json.get('performanceWork', []) if item.get('eventCast') is not None) or \
       any(act.get('performanceWork', []) for act in result_json.get('sectionsOrActs', []) if any(item.get('eventCast') for item in act.get('performanceWork', []) if item.get('eventCast') is not None)):
        print(f"\n----------------开始演职人员格式化----------------------------\n")
...
def process_multi_events(events, shows_list, md_file_path, logger):
    ...
    for event in events:
        event['PerformanceEvent']['performanceWork'] = {
            'content': event['PerformanceEvent'].get('performanceWork', []),
            'metadata': {
                "model_name": model_name,
                "timestamp": datetime.now().isoformat()
            }
...
def structured_shows_list(image_folder, logger):
    ...
    for i, row in df.iterrows():
        ...
        try:
            ...
            if shows_list_json == {"performanceWork": []}:
