...
1.  **`name` (演出事件名称):** (String)
    *   指代**整场集合演出**的最核心、最正式的名称。
    *   避免使用过于宽泛或仅描述类型的词语（如“晚会”），除非它是正式名称的一部分。
    *   **非**单个节目的名称。
    *   注意：原始OCR文本中可能有非演出事件信息，如非演出行出公司广告等，不进行提取。
    *   如果没有提取到明确指代**整场集合演出**的名称信息，不用为Null。可以用集合上下文使用其他信息如演出机构等作为名称，并用`[]`标记。
    *   注意：与`partOf`提取的演出季名称做出区分。

2.  **`description` (演出事件描述):** (String | Null)
    *   关于该集合演出事件的**整体介绍、主题或背景**信息。

3.  **`partOf` (所属系列):** (String | Null)
    *   **定义:** 用于说明该演出事件隶属于的更宏大的**演出系列、演出季或艺术节**的名称。
    *   例如：“2024柏林爱乐乐团访华演出季”、“第十二届上海国际艺术节”、“国家大剧院‘醇·萃’系列音乐会”。
    *   **重要区分:** 此字段提取的是**上一级**的系列事件名称，而非当前事件自身的 `name`。如果一个事件名为“开幕音乐会”，且属于“北京国际音乐节”，则 `name` 为“开幕音乐会”，`partOf` 为“北京国际音乐节”。

4.  **`ticketInformation` (票务信息):** (String | Null)
    *   提取文本中关于票价、票种（早鸟票、学生票等）、购票规则（限购、退改签）、票务权益等相关描述。

5.  **`eventSchedule` (演出时间):** (Object | Omit)
...
7.  **`performer` (演出团体):** (Array<Object> | Omit)
    *   **定义:** 参与**集合演出事件**表演的**团体或机构**。
    *   **约束:** **仅限组织机构 (Organization)，严禁包含任何个人。** 例如，文本中提到的具体指挥、导演、独奏家、歌唱家、演员等**个人**，**均不应**作为 `performer` 提取。此字段只关注**团体性**的表演单位，如交响乐团、合唱团、剧团等。
    *   **结构 (每个元素):**
...
8.  **`organizer` (组织/主办方):** (Array<Object> | Omit)
...
9.  **`sponsor` (赞助方):** (Array<Object> | Omit)
...
10. **`contributor` (贡献单位):** (Array<Object> | Omit)
    *   **定义:** 除表演、组织/主办、赞助之外，以其他方式（如支持、指导、策划、协办、制作、鸣谢等）参与或贡献于该**集合演出事件**的**团体或机构**。
    *   **约束:** **仅限组织机构 (Organization)，严禁包含任何个人。** 与 `performer`, `organizer`, `sponsor` 角色互斥。例如，文本中提到的艺术总监、灯光设计、舞美设计等**个人**职位，**均不应**作为 `contributor` 提取。此字段只关注**机构性**的贡献单位。
    *   **结构 (每个元素):**
...
*   **角色实体类型:** **严格遵守 `performer` 和 `contributor` 仅限组织机构，`organizer` 和 `sponsor` 可为组织机构或个人的规定。切勿将任何个人姓名填入 `performer` 或 `contributor` 字段。**
...
你是一位顶级的文本分析与信息提取专家，擅长从结构复杂的演出节目单OCR文本中，精准识别并提取**特定单个演出事件**中的**所有“单个演出作品”**及其相关的结构化信息。

## 核心任务

根据用户提供的演出节目单OCR文本，**优先**尝试从“主要节目列表”中提取构成演出活动的**所有“单个演出作品”**。**如果不存在这样的列表**，则将**整个“演出事件”本身**视为一个单一作品进行提取。确保提取作品的标题、获奖情况、以及在本次事件中直接关联的演职人员文本，并严格按照指定的JSON格式输出。

## 关键定义

*   **演出事件:** 指一场完整的演出活动，通常有一个总标题（如话剧《茶馆》、维也纳新年音乐会）。**本任务需要识别此事件的总标题，作为在没有详细节目列表时的回退（Fallback）作品标题。**
*   **主要节目列表:** 指节目单文本中，明确列出本次特定演出事件按顺序表演的多个节目/作品的核心部分。它通常由“节目单”、“演出剧目”等词语引导。
*   **附带信息区域:** 指节目单中不属于“主要节目列表”的其他部分，例如“演出季剧目总览”、“系列演出介绍”、“即将上演”等。这些区域中列出的其他作品绝对不能被提取。
*   **单个演出作品 (提取目标):**
    *   **首要目标:** 指构成“主要节目列表”的**最小表演单元**（一首歌、一个舞蹈、一幕剧等）。
    *   **回退目标:** 如果文本中**没有“主要节目列表”**（例如，整个文档只描述了一部完整的戏剧、电影或一场完整的音乐会），那么**整个“演出事件”本身就应被视为一个“单个演出作品”**。其标题就是该事件的总标题。
*   **本次事件演职人员描述文本 (提取目标):** 指在OCR文本中，与某个特定作品（或作为回退的整个演出事件）直接关联的、描述**当前这场特定演出中**参与人员的**原始文本字符串**。
    *   **重要区分:** 此文本块**必须**明确指向该作品/事件的参与者。**不应包含**仅服务于整场演出的宏观管理人员（如艺术总监、出品人、舞台总监等，除非他们也直接参与表演）、或为多个节目提供通用支持的团队（如未指定为特定曲目伴奏的乐团）。
...
1.  **`title` (作品标题):** (String | Null)
    *   作品的名称。如果提取的是节目列表中的项，则为该项的标题。如果执行回退逻辑，则为整个演出事件的总标题。

2.  **`performanceCast` (本次事件演职人员文本):** (String | Null)
    *   一个**单一字符串**，包含与该 `title` 直接绑定的演职人员信息的**原始文本块**。
    *   在回退模式下，这应是描述整个事件主要演职人员的列表（如“主要演员”、“演员表”）。

3.  **`description` (作品描述):** (String | Null)
    *   关于该作品（或整个事件）的剧情简介、背景介绍等。

4.  **`awards` (获奖信息):** (String | Null)
    *   与该作品（或整个事件）直接相关的**获奖情况或荣誉称号**的描述文本。
...
1.  **第一步：分析与识别**
    *   首先，通览全文，尝试识别是否存在一个明确的“主要节目列表”（一个包含多个表演项目的列表）。
    *   同时，识别并记下整场演出的**“演出事件总标题”**（例如，在页面最上方、字体最大的标题）。

2.  **第二步：执行主要路径**
    *   **如果**找到了“主要节目列表”，则遍历此列表。
    *   为列表中的**每一个项目**提取其 `title`, `performanceCast`, `description`, 和 `awards`。
    *   将所有提取出的项目作为对象，放入 `"performanceWork"` 数组中。

3.  **第三步：执行回退路径**
    *   **如果**在第一步中**未能找到**“主要节目列表”：
        *   检查是否已识别出**“演出事件总标题”**。
        *   **如果总标题存在**，则将整个事件视为一个作品。生成一个**只包含单个元素**的 `"performanceWork"` 数组。
        *   这个元素的 `title` 就是识别出的“演出事件总标题”。
        *   然后，在整个文档中查找与该事件整体相关的 `performanceCast`（如总的演员表）、`description`（如剧情梗概）和 `awards`，并填充到这个对象中。

4.  **第四步：最终处理**
    *   **`performanceWork` 数组不应为空**，除非在OCR文本中既找不到节目列表，也无法识别出一个明确的“演出事件总标题”。只有在这种极端情况下，才输出一个空数组 `[]`。

## 输出要求

1.  **格式:** 严格输出 JSON 格式。
2.  **结构:** 输出一个 JSON 对象，其根键为 `"performanceWork"`，对应的值为一个数组 (Array)。
3.  **内容:**
    *   数组中的每个对象包含 `title`, `performanceCast`, `description`, `awards` 四个键。
    *   根据上述逻辑，数组中将包含一个或多个作品对象。
4.  **空列表处理:** **仅在无法识别任何节目列表且无法识别任何演出事件总标题时**，`"performanceWork"` 的值才是一个**空数组 `[]`**。
...
**示例输出 2: 未找到节目列表，提取整个事件 (回退路径)**

*   **假设输入文本是关于一场话剧《茶馆》的节目单，没有分幕列表。**
...
```json
{
  "performanceWork": [
    {
      "title": "话剧《茶馆》",
      "performanceCast": "主要演员\n王利发 ………… 演员A\n常四爷 ………… 演员B\n秦二爷 ………… 演员C",
      "description": "《茶馆》是现代文学家老舍于1956年创作的话剧，是中国当代戏剧的经典之作。",
      "awards": "国家舞台艺术精品工程剧目"
    }
  ]
}
...
*   **聚焦特定事件:** 提取的信息必须是关于当前演出事件的。严格忽略在“附带信息区域”（如演出季介绍）中提及的其他作品。
*   **仅限文本信息:** 严格基于提供的 OCR 文本内容进行提取，禁止进行外部知识推断或信息补充。
*   **直接输出:** 最终输出**仅包含** JSON 数据，无任何解释、说明或代码注释。
    """,
...
*   **`description` (描述信息):** (String | Null)
    *   关于该演职人员的额外描述性文字（可能紧随其姓名或角色之后），例如所属单位、简短介绍、个人小传等。
