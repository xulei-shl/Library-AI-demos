...
def retry_with_fallback(max_attempts: int = 3, initial_wait: float = 1):
    """
...
def get_llm_response(
    ...
    参数优先级说明(从高到低):
    - model_name/temperature/base_url/api_key: 1.显式传入值 > 2.Langfuse提示词config(当use_langfuse_prompt=True，缺省默认为True) > 3.函数默认值
    - system_prompt: 1.显式传入值 > 2.Langfuse提示词(当use_langfuse_prompt=True) > 3.None
    
    特别说明，重试时的参数覆盖规则：
    - Langfuse config中的配置（model_name/temperature/base_url/api_key）仅在首次调用时生效
    ...
    Args:
        ...
        model_name (str): 模型名称
        system_prompt (str): 系统提示词，如果为 None 且 use_langfuse_prompt 为 True 则使用 Langfuse prompt
        use_langfuse_prompt (bool): 是否使用 Langfuse prompt
    ...
    if system_prompt is None and use_langfuse_prompt:
        prompt = Langfuse().get_prompt("最大算力")
        # 使用prompt.compile()动态编译提示词
        final_system_prompt = prompt.compile()
        # 从prompt.config获取模型配置(如果存在)
        if hasattr(prompt, 'config') and prompt.config:
            model_name = prompt.config.get('model', model_name)
            temperature = prompt.config.get('temperature', temperature)
            base_url = prompt.config.get('base_url', base_url)
            api_key = prompt.config.get('api_key', api_key)
            print(f"\n=== 从Langfuse提示词获取模型配置===\n")
    ...
    try:
        ...
        completion = client.chat.completions.create(
            ...
            langfuse_prompt=prompt if use_langfuse_prompt and 'prompt' in locals() else None  # 链接提示版本到追踪
        )
        return completion
    except Exception as e:
        print(f"\n=== API调用错误===\n")
...
# 使用函数
user_question = "大模型应用一般需要有哪些工程基础组件？API管理、API调用监控，提示词版本管理？"

# 使用Langfuse 提示词`最大算力`的Config配置
