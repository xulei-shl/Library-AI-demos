...
*   **底层提取原则：内容与形式分离 (Separation of Content and Form)**
    *   此字段的核心任务是识别和列出构成火花画面的**视觉形式类别**（the "form"），而不是描述这些形式的具体**内容**（the "content"）。
    *   **形式 (Form):** 指的是事物的类别或视觉构成，例如：“人物肖像”、“动物插图”、“建筑”、“徽章”、“印章”、“文本块”。
    *   **内容 (Content):** 指的是形式所承载的具体信息，例如：肖像画的是“李逵”（`Description` 的范畴），文本块里写的是“北京市火柴厂”（`Text Content` 的范畴）。
    *   **核心指令：** `Elements` 字段负责回答“画面上有什么**类型**的东西？”，而 `Description` 和 `Text Content` 字段负责回答“这些东西具体是/写了**什么**？”。
*   **严谨定义:**
    构成火花**印刷画面**的、离散的、可识别的**视觉组件和形式类别**的列表。此列表应侧重于对画面构成元素的**分类**，而非对其内容的转录或详细描述。
*   **VLM提取视角:**
    使用对象检测和图像分类能力，首先将画面分解为几个主要的视觉构成部分（如插图区、文本区、标志区等），然后识别并列出这些部分所属的通用类别。
*   **【关键区别规则：视觉元素 vs. 文本内容】**
    *   **目标：** 严格区分“识别出有文字”和“识别出文字写了什么”。
    *   **规则 1：** 当识别到文本时，`Elements` 列表中应只包含一个**通用的类别名称**，例如 `"汉字文本"`, `"英文字母"`, `"数字"`, `"标语"` 或简称 `"文本"`。
    *   **规则 2：** **绝对禁止**将OCR识别出的具体文字内容放入 `Elements` 列表中。所有文字的转录工作**必须且只能**在 `Text Content` 字段中完成。
*   **【关键排他规则】**
    *   **仅限印刷内容：** 必须严格排除任何不属于火花原始设计和印刷的部分。
    *   **必须排除的对象：** **尺子、色标卡、钱币、手、桌面纹理、背景布料**等所有用于拍摄辅助或展示尺寸的外部物品，**绝对不能**被列为“元素”。
*   **数据类型:** `List<String>`
