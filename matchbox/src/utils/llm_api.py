...
def load_settings(path: str = "config/settings.yaml") -> Dict[str, Any]:
    # 兜底加载 .env 环境变量（避免某些入口未调用 main.load_dotenv）
    try:
...
def _truncate_middle(s: str, head_len: int = 100, tail_len: int = 100) -> str:
    """
    截断字符串，保留前 head_len 和后 tail_len 字符，中间用省略号连接。
    """
    if s is None:
        return ""
    total_len = len(s)
    if total_len <= head_len + tail_len:
        return s
    return s[:head_len] + "...(省略 " + str(total_len - head_len - tail_len) + " 字符)..." + s[-tail_len:]
...
def invoke_model(
    ...
    *,
    ...
    temperature = task_cfg.get("temperature", 0.45) if temperature is None else temperature
    top_p = task_cfg.get("top_p", 0.9) if top_p is None else top_p
    ...
    for use_secondary in (False, True):
        ...
        usr_head_tail = int(log_cfg.get("user_preview_max_len", 100))
        rsp_max = int(log_cfg.get("response_preview_max_len", 2000))

        # 获取系统提示词文件名（不显示内容）
        system_prompt_file = task_cfg.get("system_prompt_file", "未配置")

        # 提取用户提示词并截断（显示前后各100字符）
        user_full = _extract_user_text(messages)
        user_preview = _truncate_middle(user_full, usr_head_tail, usr_head_tail)
        ...
        for attempt in range(1, max_retries + 1):
            ...
            try:
                ...
                # 审计日志（截断）
                user_preview_max_len = int(settings.get("logging", {}).get("user_preview_max_len", 100))
                user_prompt_full = _extract_user_preview(messages)
                user_prompt_preview = _truncate_middle(user_prompt_full, user_preview_max_len, user_preview_max_len)
                logger.info(
